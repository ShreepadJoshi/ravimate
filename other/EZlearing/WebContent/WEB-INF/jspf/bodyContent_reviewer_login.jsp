...
<%@page import="com.education.transferobj.QuestionBankTO"%>	
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="10" bgcolor="#F0F0FF">
	...
	<tr>
		...
		<td colspan="2" width="10"> 
			<label>
			<html:form action="/reviewer" styleId="reviewerListFormID" method="post">
				<html:select property="sch_classType" name="QuestionListBean"  styleClass="input_field" >
           			<html:optionsCollection property="classTypeOptions" label="label" value="value" />
          		</html:select>
			</label></td>
		
		<td colspan="1" height="30" align="right" valign="middle">
			<a onclick="javascript:document.getElementById('reviewerListFormID').submit();">
...
<display:table id="row" name="QuestionListBean.pgSearchResults" 
	 requestURI="/reviewer.do"
	 decorator="com.education.displaytag.ReviewerQuestionListTableDecorator" style="width:100%">
	
	<display:column property="questionId" title="<input type='checkbox' id='allchkBoxID' onclick='checkAll(this.id,\"hiddenIdList\")'/>" />
	<display:column title="Question Description" property="question" maxLength="50" style="text-align:left;"/>
	...
	<display:footer>
		...
		<html:submit property="action" value="LinkToClass" styleClass="url"/>	
	</display:footer>	
</display:table>
</html:form>
</td></tr>
