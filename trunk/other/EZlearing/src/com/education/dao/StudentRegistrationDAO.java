...
public class StudentRegistrationDAO extends AbstractDAO{
	...
	public List<ClassCertTO> getClassCertificateList()throws BaseAppException {
		List<ClassCertTO> ClassCertTOList = new ArrayList<ClassCertTO>();
		
		Connection con = null;
		PreparedStatement psmt = null;
		String sql = " SELECT ClassCertId,CertificateName FROM t_class_cert";
		...
		try {
			...
			while (rs.next()) {
				...
				ClassCertTOList.add(classCertTO);
			} 
		} catch (SQLException e) {
			throw new RuntimeException(e);
		} finally {			
