<%@taglib uri="http://struts.apache.org/tags-html" prefix="html" %>
<link rel="shortcut icon" href="images/favicon1.gif"/>

<%String userAgent = request.getHeader("User-Agent");
	if(userAgent.indexOf("MSIE") != -1) {%>
	<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="791" height="211" id="FlashID" title="banner" >
	 <param name="movie" value="images/flash1.swf" />
      <param name="quality" value="high" />
      <param name="wmode" value="opaque" />
      <param name="swfversion" value="8.0.35.0" />
      <param name="expressinstall" value="Scripts/expressInstall.swf" />
	      <object type="application/x-shockwave-flash" data="images/flash1.swf" width="791" height="1">
	        <param name="quality" value="high" />
	        <param name="wmode" value="opaque" />
	        <param name="swfversion" value="8.0.35.0" />
	        <param name="expressinstall" value="Scripts/expressInstall.swf" />
	      </object>
      
    </object>
<%} else if(userAgent.indexOf("Mozilla") != -1) {%>
	<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="791" height="211" id="FlashID" title="banner" >
	 <param name="movie" value="images/flash1.swf" />
      <param name="quality" value="high" />
      <param name="wmode" value="opaque" />
      <param name="swfversion" value="8.0.35.0" />
      <param name="expressinstall" value="Scripts/expressInstall.swf" />
	      <object type="application/x-shockwave-flash" data="images/flash1.swf" width="791" height="211">
	        <param name="quality" value="high" />
	        <param name="wmode" value="opaque" />
	        <param name="swfversion" value="8.0.35.0" />
	        <param name="expressinstall" value="Scripts/expressInstall.swf" />
	      </object>
      
    </object>
<%} %>
     